...
import { criarAlimentoConsumidoCompletoSchema } from '../schemas/alimentoConsumido/criarAlimentoConsumidoCompletoSchema';
...
export default class AlimentoConsumidoController {
   ...
   public async cadastrarAlimentosConsumidos(req: Request, res: Response): Promise<JsonReponseSucesso> {
      let resultadoParse: any = criarAlimentoConsumidoCompletoSchema.safeParse(req.body)
      if (!resultadoParse.success) {
         JsonReponseErro.lancar(400, 'JSON inválido', resultadoParse.error);
      }
      const retornoCriarAlimentoConsumido = await this.alimentoConsumidoService.cadastrarAlimentosConsumidos(resultadoParse.data);
